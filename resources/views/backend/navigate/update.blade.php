...
<div class="container">
	...
	<form action="{{ route('admin.navigate.update', $item->id) }}" method="POST" enctype="multipart/form-data" class="card p-3">
		@csrf @method('PUT')
		...
		<div class="row g-3">
			...
			<div class="col-md-3">
				...
				<input type="number" name="position" class="form-control" value="{{ old('position', $item->position) }}">
			</div>
			...
			<div class="col-12">
				...
				@if($item->image)
					<div class="mt-2">
						<img src="{{ asset('uploads/navigate/'.$item->image) }}" style="width:140px;height:90px;object-fit:cover;border-radius:6px">
					</div>
				@endif
			</div>
					...
					@foreach($opts as $name => $label)
						<div class="col-md-4">
							<div class="form-check">
								<input class="form-check-input" type="checkbox" name="{{ $name }}" id="{{ $name }}" value="1" {{ $item->{$name} ? 'checked' : '' }}>
